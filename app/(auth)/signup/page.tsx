...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isSupabaseConfigured()) {
      toast("Supabase not configured — demo account created. Redirecting.", "info");
    ...
    if (data.user) {
      await supabase.from("profiles").upsert({ id: data.user.id, full_name: fullName, email, role: "user", charity_percentage: 10 });
    }
    toast("Account created! Please check email if confirmation required.", "success");
    router.push("/dashboard");
  }
