  ...
  return (
    <div className="space-y-6">
      <DarkCard className="p-7 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-hidden relative">
        <div className="absolute -right-10 -top-10 h-48 w-48 rounded-full bg-[#D9FF82]/15 blur-3xl pointer-events-none" aria-hidden />
      ...
      <div className="grid md:grid-cols-3 gap-5">
        <Card className="p-5">
          <p className="text-xs font-semibold tracking-[0.12em] uppercase text-[#8D8D98]">Subscription</p>
          ...
          <p className="text-xs leading-5 text-[#8D8D98] mt-2">Renewal: {subscription?.current_period_end ? new Date(subscription.current_period_end).toLocaleDateString() : "—"}</p>
        ...
        <Card className="p-5">
          <p className="text-xs font-semibold tracking-[0.12em] uppercase text-[#8D8D98]">Performance</p>
          <p className="mt-3 text-[28px] font-bold tracking-[-0.02em] leading-none text-[#111113]">{avg}</p>
          <p className="text-xs text-[#8D8D98] mt-1">Average of last {scores.length} scores</p>
          <div className="mt-3 flex gap-1.5 flex-wrap">
            {scores.length === 0 ? (
              <span className="text-xs text-[#8D8D98]">No scores yet — add your first!</span>
            ) : (
              scores.slice(0, 5).map((s: any) => (
                <span key={s.id} className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#111113] text-white text-xs font-bold">
                  {s.score}
                </span>
              ))
            )}
        ...
        <Card className="p-5">
          <p className="text-xs font-semibold tracking-[0.12em] uppercase text-[#8D8D98]">Impact</p>
          <p className="mt-3 text-sm font-semibold tracking-[-0.01em] text-[#111113]">{selectedCharity?.name ?? "Choose a charity"}</p>
          <p className="text-xs text-[#8D8D98] mt-1">{profile?.charity_percentage ?? 10}% of subscription</p>
          <div className="mt-3 h-2 rounded-full bg-[#E8E8E3] overflow-hidden">
            <div className="h-full bg-[#D9FF82]" style={{ width: `${Math.min(profile?.charity_percentage ?? 10, 100)}%` }} />
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Card className="p-6">
          ...
          {scores.length === 0 ? (
            <p className="text-[13.5px] text-[#8D8D98] mt-2">No scores yet.</p>
          ) : (
            <div className="mt-4 space-y-2">
              {scores.map((s: any, i: number) => (
                <div key={s.id} className="flex items-center justify-between rounded-xl border border-[#E8E8E3] px-3.5 py-2.5 bg-white">
